...
from backend.request import get_session
...
def get_http_server_url():
    server_url: str = 'http://' + cm.get(ConfigField.SERVER_URI)
    http_prefix = cm.get(ConfigField.HTTP_PREFIX)
    if http_prefix is not None:
        return server_url + '/' + http_prefix
    return server_url


def url(path):
    return get_http_server_url() + path
...
def room_start(position):
    get_session().get(url('/room/play'), params={"position": position})


def room_stop():
    get_session().get(url('/room/stop'))


def room_pause(position):
    get_session().get(url('/room/pause'), params={"position": position})


def room_seek(position):
    get_session().get(url('/room/seek'), params={"position": position})


def room_program_set(programs, client_ids=None):
    get_session().get(url('/room/program/set'), params={
...
def room_leave():
    get_session().get(url('/room/leave'))


def room_get():
    return get_session().get(url('/room/get')).json()["data"]


def room_join(room_id):
    return get_session().get(url('/room/join'), params={"roomId": room_id})


def room_create(room_name, secret=False):
    return get_session().get(url('/room/create'), params={"name": room_name, 'secret': secret})


def room_list():
    return get_session().get(url('/room/list')).json()
